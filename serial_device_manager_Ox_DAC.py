...
global serverNameAD5764_ACBOX; serverNameAD5764_ACBOX = "acbox" # ad5764 acbox
...
class serialDeviceManager(object):
	...
	def identifyPort(self,port):
		...
		elif response.startswith('DAC-ADC_AD7734-AD5764'):
			print('\tPort %s identified as a DAC_ADC_AD7734-AD5764 device.'%port)
			self.regWrite("dac_adc",response,port)
			return True

		elif response.startswith('DAC-ADC_AD7734-AD5780'):
			print('\tPort %s identified as a DAC_ADC_AD7734-AD5780 device.'%port)
			self.regWrite("dac_adc",response,port)
			return True
		...
		print("\tTrying Other devices: SIM900, Mercury ITC and Mercury IPS")

		simrate = 115200
		time.sleep(IO_DELAY)

		self.ser.baudrate(simrate)
		...
		#self.ser.write('CONN 4,"xyz"\n')
		#time.sleep(IO_DELAY)
		#connresponse = self.ser.read_line()
		#print(connresponse)
		self.ser.write("*IDN?\r\n")            # query identification command
		time.sleep(IO_DELAY)                   # delay for processing
		response = self.ser.read_line()        # get response from device
		if response:
			...
			print("no response acquired")

		if response.startswith('Stanford_Research_Systems,SIM900'):
			print('SIM900 was identified')
			self.regWrite("SIM900",response,port)
			return True
		elif response.startswith('IDN:OXFORD INSTRUMENTS:MERCURY IPS'):
			print('Mercury IPS was identified')
			self.regWrite("Mercury IPS Server",response,port)
			return True 
		elif response.startswith('IDN:OXFORD INSTRUMENTS:MERCURY ITC'):
			print('Mercury ITC was identified')
			self.regWrite("Mercury ITC Server",response,port)
			return True
		else:
			print("Nothing was not identified")
		...
		# print("\tTrying device type: Oxford IPS")
		...
		# if response.startswith('IDN:OXFORD INSTRUMENTS:MERCURY IPS'):
		# 	print('Mercury IPS was identified')
		# 	self.regWrite("Mercury IPS Server",response,port)
		# 	return True 
		# else:
		# 	print("Mercury IPS was not identified")
		# 	self.regWrite("Mercury ITC Server",response,port)
		# 	return True
		...
		# print("\tTrying device type: Oxford ITC")
		...
		# if response.startswith('IDN:OXFORD INSTRUMENTS:MERCURY ITC'):
		# 	print('Mercury ITC was identified')
		# 	self.regWrite("Mercury ITC Server",response,port)
		# 	return True
		# else:
		# 	print("Mercury ITC was not identified")
