...
# Copyright (C) 2026 Polshyn Lab, Gregory Polshyn (improving documentation)
# Modified: 2026-09-09
# Based on earlier DAC-ADC / GateKeeper server implementations.
#
...
class DAC_ADCServer(DeviceServer):
    ...
    def set_voltage(self,c,port,voltage):
        """
        set_voltage(port, voltage)

        Set one DAC output voltage.

        Parameters:
            port : int: DAC channel, 0 through 7.
            voltage : float: Target DAC voltage in volts; must be between -10 V and +10 V.

        Returns:
            str: Firmware status string, for example "DAC 0 UPDATED TO 0.999996 V".

        Example:
            Applying 1.0 V to output 0.
    ...
    def read_voltage(self,c,port):
        ...
        Read one ADC input channel once.

        Parameters:
            port : int: ADC channel, 0 through 7.

        Returns:
            float: Measured ADC voltage in volts.

        Example:
            Reading ADC input 0.
            voltage = dac.read_voltage(0)
        """
        dev=self.selectedDevice(c)
        if not (port in range(8)):
            returnValue("Error: invalid port number.")
    ...
    def ramp1(self,c,port,ivoltage,fvoltage,steps,delay):
        """
        ramp1(port, ivoltage, fvoltage, steps, delay)

        Ramp one DAC channel linearly.

        Parameters:
            port : int: DAC channel, 0 through 7.
            ivoltage, fvoltage : float: Initial and final DAC voltages in volts.
            steps : int: Number of ramp points.
            delay : int: Settling time between points in microseconds.

        Returns:
            str: Firmware ramp status string.

        Example:
            Ramping DAC output 0 from 0 V to 1 V in 101 points.
            dac.ramp1(0, 0.0, 1.0, 101, 10000)
        """
        dev=self.selectedDevice(c)
    ...
    def buffer_ramp(self,c,dacPorts,adcPorts,ivoltages,fvoltages,steps,delay,nReadings=1):
        """
        buffer_ramp(dacPorts, adcPorts, ivoltages, fvoltages, steps, delay, nReadings=1)

        Ramp selected DACs while reading selected ADCs.

        DAC channels in dacPorts are swept from matching ivoltages to
        fvoltages over steps points. At each point, ADC channels in
        adcPorts are read nReadings times and averaged.
        For additional control of the settling time, see
        dac_led_buffer_ramp. This function wraps firmware command
        DAC_LED_BUFFER_RAMP.

        Parameters:
            dacPorts : list[int]: DAC channels to ramp.
            adcPorts : list[int]: ADC channels to read; also sets returned channel order.
            ivoltages, fvoltages : list[float]: Initial and final DAC voltages in volts, ordered like dacPorts.
            steps : int: Number of ramp points, including endpoints when steps > 1.
            delay : float: DAC interval in microseconds. May be increased if ADC conversion and averaging cannot fit in the requested interval.
            nReadings : int, optional: ADC readings to average per ramp point. Defaults to 1.

        Returns:
            list[list[float]]: ADC traces in volts: one steps-long list per adcPorts entry.

        Example:
            Ramp DAC 0 from 0 V to 1 V in 101 points while reading ADCs 0 and 1.
            data = dac.buffer_ramp([0], [0, 1], [0.0], [1.0], 101, 10000.0, 1)
        """
    ...
    def dac_led_buffer_ramp(self,c,dacPorts,adcPorts,ivoltages,fvoltages,steps,dacInterval,dacSettlingTime,nReadings=1):
        """
        dac_led_buffer_ramp(dacPorts, adcPorts, ivoltages, fvoltages, steps, dacInterval, dacSettlingTime, nReadings=1)

        Ramp selected DACs and read selected ADCs after a fixed settling time.

        DAC channels in dacPorts are swept from matching ivoltages to
        fvoltages over steps points. At each point, the firmware waits
        dacSettlingTime after the DAC update, then reads ADC channels in
        adcPorts nReadings times and averages them. This function wraps
        firmware command DAC_LED_BUFFER_RAMP.
        ...
        Parameters:
            ...
            ivoltages, fvoltages : list[float]: Initial and final DAC voltages in volts, ordered like dacPorts.
            steps : int: Number of ramp points, including endpoints when steps > 1.
            dacInterval : float: Time between DAC updates in microseconds.
            dacSettlingTime : float: Time to wait after each DAC update before ADC acquisition, in microseconds. Must be shorter than dacInterval.
            nReadings : int, optional: ADC readings to average per ramp point. Defaults to 1.
        ...
        Example:
            Ramp DAC 0 from 0 V to 1 V in 101 points and read ADCs 0 and 1 after 2 ms settling.
            data = dac.dac_led_buffer_ramp([0], [0, 1], [0.0], [1.0], 101, 10000.0, 2000.0, 1)
        """
        dacN = len(dacPorts)
        adcN = len(adcPorts)
    ...
    def generate_awg(self,c,dacPorts,voltageLists,dacInterval):
        """
        generate_awg(dacPorts, voltageLists, dacInterval)

        Generate a repeated arbitrary waveform on selected DACs.

        Each list in voltageLists gives the waveform for the matching DAC
        channel in dacPorts. All selected DACs update together every
        dacInterval microseconds. The firmware repeats the waveform until
        stopped with stop_ramp. This function wraps firmware command
        AWG_BUFFER_RAMP and does not acquire ADC data.

        Parameters:
            dacPorts : list[int]: DAC channels to update.
            voltageLists : list[list[float]]: DAC voltages in volts, one waveform list per dacPorts entry. All waveform lists should have the same length.
            dacInterval : int: Time between waveform points in microseconds.

        Returns:
            list[list[float]]: Empty list; this setting starts the AWG but returns no ADC traces.

        Example:
            Repeating a 3-point waveform on DAC output 0 with 10 ms between points.
            dac.generate_awg([0], [[0.0, 1.0, 0.0]], 10000)
        """
        dacN = len(dacPorts)
    ...
    def awg_with_adc(self, c, dacPorts, adcPorts, voltageLists, dacInterval_us, numCycles=1):
        """
        awg_with_adc(dacPorts, adcPorts, voltageLists, dacInterval_us, numCycles=1)

        Run a repeated DAC waveform while reading ADCs at each point.

        Each list in voltageLists gives the waveform for the matching DAC
        channel in dacPorts. All selected DACs update together every
        dacInterval_us microseconds. The firmware captures one ADC frame
        from adcPorts at each DAC point. numCycles=0 runs until
        stop_ramp sends STOP. This function wraps firmware command
        AWG_WITH_ADC.
        ...
        Parameters:
            ...
            adcPorts : list[int]: ADC channels to read; also sets returned channel order.
            voltageLists : list[list[float]]: DAC voltages in volts, one waveform list per dacPorts entry. All waveform lists should have the same length.
            dacInterval_us : int: Time between waveform points in microseconds.
            numCycles : int, optional: Number of waveform repetitions. Use 0 to run until stopped. Defaults to 1.

        Returns:
            list[list[float]]: ADC traces in volts: one list per adcPorts entry. The setting also emits one sigAWGData JSON payload per ADC frame.

        Example:
            Play one 3-point waveform cycle on DAC 0 while reading ADCs 0 and 1.
            data = dac.awg_with_adc([0], [0, 1], [[0.0, 1.0, 0.0]], 10000, 1)
        """
    ...
    def time_series_buffer_ramp_2d(self, c, dacPorts, adcPorts, startPoint, fastAxisVector, slowAxisVector, stepsFast, stepsSlow, retrace, snake, dacInterval_us, adcInterval_us):
        ...
        dac_int = int(dac_interval)
        adc_int = int(adc_interval)
        points_per_line = max(1, int(stepsFast * dac_int) // adc_int)
    ...
    def get_conversion_time(self,c,channel):
        """
        get_conversion_time(channel)

        Read one ADC channel's conversion time.

        Parameters:
            channel : int: ADC channel, 0 through 7.

        Returns:
            float: Current conversion time in microseconds.

        Example:
            Reading the conversion time for ADC input 0.
            time_us = dac.get_conversion_time(0)
        """
        dev = self.selectedDevice(c)
    ...
    def set_conversion_time(self,c,channel,time):
        ...
        The firmware selects the closest supported value and returns the actual
        time set. Shorter conversion times are faster but usually noisier.

        Parameters:
            channel : int: ADC channel, 0 through 7.
            time : float: Requested conversion time in microseconds. This server accepts 82 us through 2686 us.

        Returns:
            float: Actual conversion time set, in microseconds.
    ...
    def initialize(self,c):
        ...
        Initialize GateKeeper hardware after power-up.

        Resets the ADC boards, restores saved calibration, sets ADC conversion
        times to about 500 us, initializes the DACs, and sets all DAC outputs
        to 0 V.

        Returns:
            str: Firmware status string, normally "INITIALIZATION COMPLETE".

        Example:
            Initializing GateKeeper after power-up.
            dac.initialize()
        """
        dev=self.selectedDevice(c)
