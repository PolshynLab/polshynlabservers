...
name = Mercury IPS
version = 1.1
description = Mercury Magnet Power Supply Controller
...
class IPSWrapper(DeviceWrapper):
    ...
    #Followings are added functions to read temperature
    # @inlineCallbacks
    # def read_magnet_temperature(self, c):   # Read the temperature of MB1
        # dev=self.selectedDevice(c)
    ...
    # @inlineCallbacks
    # def read_PT1_temperature(self, c):   # Read the temperature of PT1
        # dev=self.selectedDevice(c)
    ...
    # @inlineCallbacks
    # def read_PT2_temperature(self, c):   # Read the temperature of PT2
         # dev=self.selectedDevice(c)
...
class IPSServer(DeviceServer):
    ...
    def loadConfigInfo(self):
        reg = self.reg
        yield reg.cd(['', 'Servers', 'Mercury IPS', 'Links'], True)
    ...
    def ramp_status(self, c):
        dev=self.selectedDevice(c)
        yield dev.write("READ:DEV:GRPZ:PSU:ACTN\n")
        ans = yield dev.read()
        ANS = ans.split(':')
        ans = ANS[-1]
        # ans = ans[:-1]
    ...
    #Followings are added functions to read temperature
    @setting(546, returns= 's')          # Try to read the temperature of sensor
    def read_magnet_temperature(self, c):
        dev=self.selectedDevice(c)
        yield dev.write("READ:DEV:MB1.T1:TEMP:SIG:TEMP\n")
        ans = yield dev.read()
        ANS = ans.split(':')
    ...
    @setting(547, returns= 's')          # Try to read the temperature of sensor
    def read_PT1_temperature(self, c):
        dev=self.selectedDevice(c)
        yield dev.write("READ:DEV:DB8.T1:TEMP:SIG:TEMP\n")
        ans = yield dev.read()
        ANS = ans.split(':')
    ...
    @setting(548, returns= 's')          # Try to read the temperature of sensor
    def read_PT2_temperature(self, c):
        dev=self.selectedDevice(c)
        yield dev.write("READ:DEV:DB7.T1:TEMP:SIG:TEMP\n")
        ans = yield dev.read()
        ANS = ans.split(':')
