...
### BEGIN NODE INFO
[info]
name = Mercury ITC Heliox
version = 1.2
description = Oxford Temperature Controller for Heliox
...
class ITCServer(DeviceServer):
    ...
    def loadConfigInfo(self):
        reg = self.reg
        yield reg.cd(['', 'Servers', 'Mercury ITC Heliox', 'Links'], True)
    ...
    def vti_temperature(self, c):
        ...
        """
        dev=self.selectedDevice(c)
        yield dev.write("READ:DEV:DB6.T1:TEMP:SIG:TEMP"+"\n")
        ans = yield dev.read()
        ANS = ans.split(':')
        ans = ANS[-1]
        ans = ans[:-1]
    ...
    @setting(526, 'Probe temperature low', returns='v')
    def probe_temperature_low(self, c):
        """Query the value of the probe temperature LOW - RuOx sensor

        Returns:
            (Value[K]): Probe temperature LOW.
        """
        dev=self.selectedDevice(c)
    ...
    @setting(528, 'Probe temperature high', returns='v')
    def probe_temperature_high(self, c):
        """Query the value of the probe temperature HIGH -Cernox sensor

        Returns:
            (Value[K]): Probe temperature HIGH.
        """
        dev=self.selectedDevice(c)
        yield dev.write("READ:DEV:DB7.T1:TEMP:SIG:TEMP"+"\n")
        ans = yield dev.read()
        ANS = ans.split(':')
        ans = ANS[-1]
        ans = ans[:-1]
    ...
    def probe_temperature_setpoint(self, c, setpoint=None):  
    ...
    def vti_temperature_ramp_rate(self, c, ramp_rate=None):
        ...
        dev=self.selectedDevice(c)
        if ramp_rate is not None:
            yield dev.write("SET:DEV:DB6.T1:TEMP:LOOP:RSET:"+str(ramp_rate)+"\n")
            ans = yield dev.read()
        yield dev.write("READ:DEV:DB6.T1:TEMP:LOOP:RSET"+"\n")
        ans = yield dev.read()
        ANS = ans.split(':')
        ans = ANS[-1]
        ans = ans[:-3]
    ...
    def ramp_vti_temperature_setpoint(self,c, ramping=None):
        ...
        if ramping is not None:
            if ramping is True:
                ...
                state = "OFF"
            yield dev.write("SET:DEV:DB6.T1:TEMP:LOOP:RENA:"+state+"\n")
            ans = yield dev.read()
        yield dev.write("READ:DEV:DB6.T1:TEMP:LOOP:RENA\n")
        ans = yield dev.read()
        ANS = ans.split(':')
        ans = ANS[-1]
        if ans=="ON":
    ...
    @setting(537, 'pressure', returns = 'v')  # changes slot to DB3
    def pressure(self, c):
        ...
        """
        dev=self.selectedDevice(c)
        yield dev.write("READ:DEV:DB3.P1:PRES:SIG:PRES"+"\n")
        ans = yield dev.read()
        ANS = ans.split(':')
        ans = ANS[-1]
        ans = ans[:-2]
    ...
    def pressure_setpoint(self, c, setpoint=None):
        ...
        dev=self.selectedDevice(c)
        if setpoint is not None:   
            yield dev.write("SET:DEV:DB3.P1:TEMP:LOOP:TSET:"+str(setpoint)+"\n")
            ans = yield dev.read()
        yield dev.write("READ:DEV:DB3.P1:TEMP:LOOP:TSET"+"\n")
        ans = yield dev.read()
        ANS = ans.split(':')
        ans = ANS[-1]
        ans = ans[:-2]
    ...
    def ramp_pressure_setpoint(self, c, ramping=None):
        ...
        if ramping is not None:
            if ramping is True:
                ...
                state = "OFF"
            yield dev.write("SET:DEV:DB3.P1:PRES:LOOP:SWMD:"+state+"\n")
            ans = yield dev.read()
        yield dev.write("READ:DEV:DB3.P1:PRES:LOOP:SWMD"+"\n")
        ans = yield dev.read()
        ANS = ans.split(':')
        ans = ANS[-1]
        if ans=="ON":
    ...
    def vti_temperature_setpoint(self, c, setpoint=None):
        ...
        dev=self.selectedDevice(c)
        if setpoint is not None:
            dev.write("SET:DEV:DB6.T1:TEMP:LOOP:TSET:"+str(setpoint)+"\n")
            ans = yield dev.read()
        yield dev.write("READ:DEV:DB6.T1:TEMP:LOOP:TSET"+"\n")
        ans = yield dev.read()
        ANS = ans.split(':')
        ans = ANS[-1]
        ans = ans[:-1]
    ...
    def probe_heater_power(self, c):
        ...
        """
        dev=self.selectedDevice(c)
        yield dev.write("READ:DEV:DB2.H1:HTR:SIG:POWR"+"\n")
        ans = yield dev.read()
        ANS = ans.split(':')
        ans = ANS[-1]
        ans = ans[:-1]
    ...
    def vti_heater_power(self, c):
        ...
        """
        dev=self.selectedDevice(c)
        yield dev.write("READ:DEV:DB1.H1:HTR:SIG:POWR"+"\n")
        ans = yield dev.read()
        ANS = ans.split(':')
        ans = ANS[-1]
        ans = ans[:-1]
    ...
    @setting(549, 'Sorb temperature', returns='v')
    def sorb_temperature(self, c):
        """Query the value of the sorb temperature

        Returns:
            (Value[K]): Sorb temperature.
        """
        dev=self.selectedDevice(c)
    ...
    @setting(550, 'Sorb heater power', returns='v')
    def sorb_heater_power(self, c):
        """Query the sorb heater power

        Returns:
            (Value[W]): Sorb heater power.
        """
        dev=self.selectedDevice(c)
    ...
    @setting(551, 'He4 plate temperature', returns='v')
    def he4_plate_temperature(self, c):
        """Query the value of the He4 plate temperature

        Returns:
            (Value[K]): He4 plate temperature.
        """
        dev=self.selectedDevice(c)
        yield dev.write("READ:DEV:DB5.T1:TEMP:SIG:TEMP"+"\n")
        ans = yield dev.read()
        ANS = ans.split(':')
        ans = ANS[-1]
        ans = ans[:-1]
