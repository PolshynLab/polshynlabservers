...
class K2450(GPIBManagedServer):
    ...
    def read_i(self, c):
        dev = self.selectedDevice(c)
        data = yield dev.query('MEAS:CURR:DC?')
        #current = (data.split(',')[1] )   Something went wrong with this line. With this line " current = (current.split(',')[1] ) builtins.IndexError: list index out of range" would occur
        current = data  
        returnValue( float(current) )
